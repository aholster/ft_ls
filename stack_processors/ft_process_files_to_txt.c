...
/*   Updated: 2019/11/12 05:43:40 by aholster      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
// 	ft_find_longest_fields(*afinfo_stack, &min_fields, amax_len, aflags);
...
{
	int			ret;
	t_fvec_queue	intermediate;

	ft_bzero(&intermediate, sizeof(t_fvec_queue));
	if (ft_create_intermediate(*afinfo_stack, &intermediate, aflags) == -1)
	{
		return (-1);
	}
	else
	{
		if (((*aflags) & flg_l) > 0)
		{
			ret = ft_long_format(aout_stack, &intermediate, amax_len, aflags);
		}
		else if (((*aflags) & flg_i) > 0)
		{
			ret = ft_inode_format(aout_stack, &intermediate, amax_len, aflags);
		}
		else
		{
			ret = ft_basic_format(aout_stack, &intermediate, amax_len, aflags);
		}
		if (ret == -1)
		{
			return (-1);
